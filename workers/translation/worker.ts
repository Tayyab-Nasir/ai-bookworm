...
async function main() {
  if (process.argv.slice(2).some((arg) => !["--once", "--quoted", "--prepare-quotes"].includes(arg)) || (process.argv.includes("--quoted") && process.argv.includes("--prepare-quotes"))) throw new Error("Usage: worker:translation [--once] [--quoted | --prepare-quotes]");
  ...
  do {
    let delay = 0;
    try { const result = await (process.argv.includes("--prepare-quotes") ? runOneTranslationQuoteStep(sb) : process.argv.includes("--quoted") ? runOneQuotedTranslationJob(sb) : runOneTranslationJob(sb)); if (result.status === "idle") delay = 2_000; else console.log(JSON.stringify(result)); }
