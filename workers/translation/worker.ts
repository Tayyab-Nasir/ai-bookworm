...
async function main() {
  if (process.argv.slice(2).some((arg) => arg !== "--once")) throw new Error("Usage: worker:translation [--once]");
  ...
  do {
    let delay = 0;
    try { const result = await runOneTranslationJob(sb); if (result.status === "idle") delay = 2_000; else console.log(JSON.stringify(result)); }
