...
| 2 | Native Supabase project and migrations | PARTIAL | The last verified live state had the first 45 repository migrations. All 71 current repository migrations execute in disposable PostgreSQL; later additions remain local-only. Native PostgREST/Storage acceptance and current drift checks remain before production. |
| 3 | Tenant isolation and least privilege | PARTIAL | Forty-nine SQL boundary/workflow suites execute locally, and 30 Python security tests pass. The disposable PostgreSQL fixture does not prove Storage HTTP, JWT claims, GoTrue or multi-connection race behavior. |
...
| 7 | DOCX, EPUB, TXT and limited PDF import fixtures | PARTIAL | Authenticated/root-confined parsers preserve supported DOCX marks/list styles, table text and explicitly marked leading header rows; EPUB nested content and header rows are imported once. Body PNG/JPEG/GIF/WebP images pass bounded extraction, independent validation/scanning, private storage and atomic chapter/asset persistence with retry receipts. Real supplied specifications parse. Merged cells, header/footer drawings, vector images, numbering/styles and scanned-PDF OCR remain incomplete. Native Storage/Postgres acceptance and orphan reconciliation are outstanding. |
| 8 | Deterministic EPUB/PDF/cover rendering | PARTIAL | EPUB 1.9.0/PDF 1.13.0 fixtures cover inline marks, nested lists, illustrations, escaped metadata, gutters, paginated tables with explicit column-header semantics, and RTL EPUB language/direction metadata. Print tables repeat designated headers on later pages; a header-bearing EPUB passes local EPUBCheck 5.4.0. Dedicated full-bleed pages have saved crop focus, physical-page placement, no visible page number and 300-DPI preflight. Edition schema 1.1.0 persists direction; render, preflight and package fingerprints use the saved language. Unsafe RTL print/cover text is blocked. Vera-selected editions embed all used fonts. Private artifacts have checksum/size verification. Tagged PDF, RTL shaping/full script coverage, merged cells/row-header associations, fixed-layout completion and physical print/crop proof remain incomplete. |
