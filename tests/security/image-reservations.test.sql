...
begin
  ...
  for i in 1..4 loop
    insert into public.ai_jobs(workspace_id,agent_type,status,input_ref,idempotency_key,created_by)
      values(case when i<4 then v_ws else v_other end,'illustrator','queued','{}','hold-'||i,v_user)
      returning id into v_job;
  end loop;
  ...
  begin
    insert into public.ai_jobs(workspace_id,agent_type,status,input_ref,idempotency_key,created_by)
      values(v_ws,'cover_designer','running','{}','over-capacity',v_user);
  ...
  insert into public.ai_jobs(workspace_id,agent_type,status,input_ref,idempotency_key,created_by)
    values(v_ws,'cover_designer','running','{}','replacement',v_user) returning id into v_replacement;
  begin
    update public.ai_jobs set status='running' where id=v_job;
