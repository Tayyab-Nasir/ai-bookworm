-- Text-only and illustrated imports share one atomic chapter/receipt boundary.
create or replace function public.complete_manuscript_import(
...
begin
  ...
  if jsonb_typeof(p_chapters) is distinct from 'array' or jsonb_array_length(p_chapters) not between 1 and 500
    ...
    or jsonb_typeof(p_images) is distinct from 'array' or jsonb_array_length(p_images) not between 0 and 100
  ...
  for v_item in select value from jsonb_array_elements(p_chapters) loop
    ...
    insert into public.document_versions(chapter_id,version_number,content_json,plain_text,word_count,created_by,change_summary)
      values(v_chapter.id,1,jsonb_build_object('schemaVersion','1.0','nodes',v_item->'nodes'),v_text,
        ...
        p_actor_id,'Manuscript imported') returning id into v_version_id;
...
$$;
revoke all on function public.complete_manuscript_import(uuid,uuid,uuid,text,jsonb,jsonb,jsonb) from public,anon,authenticated;
grant execute on function public.complete_manuscript_import(uuid,uuid,uuid,text,jsonb,jsonb,jsonb) to service_role;
-- Preserve the earlier private RPC without maintaining a second transaction body.
create or replace function public.complete_manuscript_image_import(
  ...
  select public.complete_manuscript_import(p_actor_id,p_book_id,p_source_asset_id,p_source_checksum,p_chapters,p_images,p_report);
$$;
