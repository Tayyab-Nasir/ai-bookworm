-- Extend the two service-only publishing completions for Google Play Books ebook exports.
-- Existing validation, source-fingerprint, private asset and credit rules are unchanged.
...
begin
  ...
  if v_job.status <> 'running' or v_job.created_by is null
    or v_job.channel not in ('export','kdp','apple','barnesnoble','lulu','googleplay')
...
begin
  ...
  if v_job.status = 'succeeded' then return v_job; end if;
  if v_job.status <> 'running' or v_job.created_by is null
    or v_job.channel not in ('kdp','apple','barnesnoble','lulu','googleplay')
